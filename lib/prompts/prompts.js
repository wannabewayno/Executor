...
prompts.viewByDepartment = () => {
    return makePrompt('departmentView','list','Choose a department',
        autocomplete.searchFromData(dbMap['Department'])
    );
}
...
prompts.select = dataPacket => {
    const { type, callbackName, action } = dataPacket;

    dbMap[type].formatOptions.callbackName = callbackName;

     return makePrompt(`select${type}`, 'autocomplete', `Select ${type} to ${action}`,
